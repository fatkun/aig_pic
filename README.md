...
- 🖼️ 以图生图：上传参考图片，结合提示词生成新图片
- 📋 任务队列：可配置并发任务管理，支持排队
- 🖼️ 图片管理：网格展示、分页浏览、全屏预览、删除、复用
- 💾 历史记录：SQLite 存储，支持查询提示词
- ⚙️ 配置切换：从后端配置文件加载多套 API 配置
- ⌨️ 键盘快捷键：Ctrl+Enter 生成图片，方向键翻页/切换图片

## 最近更新

- ✨ 新增以图生图功能，支持上传参考图片生成
- 🎯 任务列表优化：展开/收起（3条↔15条）、运行中任务优先排序
- 📊 任务详情：点击任务查看提示词、状态、错误信息
- 🌊 运行中任务波浪动画效果
- 🚀 图片下载优化：2线程并发 + 3次重试机制
- 🎨 图片操作：删除按钮、附件按钮（复用已生成图片）
- ⌨️ 键盘快捷键：Ctrl+Enter 生成，方向键导航
- 🐛 错误日志增强：完整堆栈跟踪、HTTP错误详情、UTF-8编码
